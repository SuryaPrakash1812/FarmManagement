...
public sealed class ExpensesController : CrudController<Expense, MoneyRecordDto, UpsertExpenseRequest> { public ExpensesController(IRepository<Expense> r) : base(r, EntityMapper.ToDto, EntityMapper.ToEntity, EntityMapper.Apply) { } protected override IQueryable<Expense> ApplySearch(IQueryable<Expense> q, string? s) => string.IsNullOrWhiteSpace(s) ? q : q.Where(x => x.Category.Contains(s) || x.PaymentMethod.Contains(s)); }
