...
function AnimalDialog({open,animal,onClose,onSaved}){ const blank={animalCode:'',tagNumber:'',name:'',species:'Cow',breed:'',gender:0,purchasePrice:0,currentValue:0,healthStatus:'Healthy',isPregnant:false,status:0}; const [form,setForm]=useState(blank); const [photo,setPhoto]=useState(null); const [preview,setPreview]=useState(''); const [error,setError]=useState(''); const video=useRef(null); const streamRef=useRef(null); useEffect(()=>{if(open){setError('');setForm(animal?{...blank,...animal}:blank);setPreview(animal?.photoUrl||'');setPhoto(null)}},[open,animal]); function patch(e){setForm({...form,[e.target.name]:e.target.value})} function pick(file){if(!file)return;setPhoto(file);setPreview(URL.createObjectURL(file))} async function startCamera(){streamRef.current=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});video.current.srcObject=streamRef.current} function capture(){const c=document.createElement('canvas');c.width=video.current.videoWidth;c.height=video.current.videoHeight;c.getContext('2d').drawImage(video.current,0,0);c.toBlob(b=>pick(new File([b],'animal-camera.jpg',{type:'image/jpeg'})),'image/jpeg',.92)} async function save(){setError(''); let saved; try{saved=animal?(await api.put(`/animals/${animal.id}`,form)).data:(await api.post('/animals',form)).data;if(photo){const fd=new FormData();fd.append('photo',photo);await api.post(`/animals/${saved.id}/photo`,fd,{headers:{'Content-Type':'multipart/form-data'}})} stop(); onSaved()}catch(err){setError(err?.response?.data?.title || err?.response?.data?.message || `Could not save (status ${err?.response?.status||'unknown'}). Check the fields and try again.`)}} function stop(){streamRef.current?.getTracks().forEach(t=>t.stop())} return <Dialog open={open} onClose={()=>{stop();onClose()}} fullWidth maxWidth="md"><DialogTitle>{animal?'Edit Animal':'Add Animal'}</DialogTitle><DialogContent>{error && <Alert severity="error" sx={{mb:2}}>{error}</Alert>}<div className="form-grid">{['animalCode','tagNumber','name','species','breed','healthStatus'].map(k=><input key={k} className="input" name={k} placeholder={label(k)} value={form[k]} onChange={patch}/>) }<input className="input" name="purchasePrice" type="number" placeholder="Purchase Price" value={form.purchasePrice} onChange={patch}/><input className="input" name="currentValue" type="number" placeholder="Current Value" value={form.currentValue} onChange={patch}/><select name="gender" value={form.gender} onChange={patch}><option value={0}>Female</option><option value={1}>Male</option><option value={2}>Unknown</option></select></div><Box sx={{mt:2}} className="photo-panel"><img className="photo-preview" src={preview||'/icon.svg'} /><div><Button component="label" startIcon={<CloudUpload/>} variant="outlined" sx={{mr:1,mb:1}}>Upload Photo<input hidden type="file" accept="image/*" onChange={e=>pick(e.target.files[0])}/></Button><Button component="label" startIcon={<CameraAlt/>} variant="outlined" sx={{mr:1,mb:1}}>Take Pic<input hidden type="file" accept="image/*" capture="environment" onChange={e=>pick(e.target.files[0])}/></Button><Button startIcon={<CameraAlt/>} variant="contained" onClick={startCamera} sx={{mb:1}}>Live Camera</Button><video ref={video} autoPlay playsInline style={{width:'100%',maxWidth:360,borderRadius:8,display:'block',marginTop:8}}/><Button className="btn secondary" onClick={capture} sx={{mt:1}}>Capture Frame</Button></div></Box></DialogContent><DialogActions><Button onClick={()=>{stop();onClose()}}>Cancel</Button><Button variant="contained" onClick={save}>Save</Button></DialogActions></Dialog>}
